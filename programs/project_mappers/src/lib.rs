...
pub mod project_mappers {
    ...
    pub fn release_payment(ctx: Context<ReleasePayment>) -> Result<()> {
        ...
        require!(
            ctx.accounts.authority.key() == client_key || ctx.accounts.authority.key() == oracle_key,
        ...
        msg!("Payment of {} lamports released.", amount);
        Ok(())
    }
    ...
    pub fn cancel_job(ctx: Context<CancelJob>) -> Result<()> {
        ...
        msg!("Job cancelled. {} lamports refunded.", amount);
        Ok(())
    }
}
...
impl GigEscrow {
    pub const MAXIMUM_SPACE: usize = 8 + 32 + 32 + 32 + 8 + 4 + 32 + 1 + 1 + 1; // 151
}
