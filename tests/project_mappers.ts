...
describe("project_mappers", () => {
  ...
  describe("initialize_job", () => {
    ...
    it("creates escrow, locks SOL in vault, stores correct state", async () => {
      ...
      assert.ok(
        vaultBalance >= JOB_AMOUNT.toNumber(),
        "vault must hold at least job amount",
      );
      assert.ok(
  ...
  describe("release_payment", () => {
    ...
    before(async () => {
      ...
      await airdrop(connection, client.publicKey, 30);
    ...
    it("oracle releases payment to freelancer and closes escrow", async () => {
      const freelancerBalanceBefore = await connection.getBalance(
      ...
      const freelancerBalanceAfter = await connection.getBalance(
        freelancer.publicKey,
      );
      assert.ok(
    ...
    it("drains the full live vault balance, not just the recorded amount", async () => {
      ...
      const freelancerBalanceBefore = await connection.getBalance(
        freelancer2.publicKey,
      );

      assert.ok(
        vaultBalanceBefore >= JOB_AMOUNT.toNumber() + extraLamports,
        "vault should hold extra lamports",
      );
    ...
    it("client can manually release payment", async () => {
      ...
      assert.ok(
        balAfter - balBefore >= JOB_AMOUNT.toNumber(),
        "freelancer2 must receive funds",
      );
    });
